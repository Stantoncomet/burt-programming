...
#define SPEED_LIMIT 400 // this limits top speed of motor for test. Actual motor range is 1100 to 1900, 
...
void simple_motors(void)
{
  ...
  int motor3_speed;
  int motor4_speed;
#define KICK_UP_SPEED  50       // move to header file later.
//  int motor0_speed;
...
#else   // original code that does not make operater happy.
        if(up && ! down)   //go up,  
        {
            if(last_speed < MAX_SPEED)  // yea we can over speed by RAMP_STEP.
                last_speed += RAMP_STEP;          
